...
These plugins provide a Wordpress interface for the Institute for Development Studies (IDS)'s Open API (http://api.ids.ac.uk/).
The goal is to allow retrieving data from the IDS Collections (Eldis and Bridge) for use within a Wordpress site.


The IDS KS API plugins allows access to IDS Knowledge Services content of thematically organised and hand selected academic research on poverty reduction in the developing world that is freely available to access online.
